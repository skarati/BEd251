...
void scalar_mult_fixed_base(unsigned char op[32], unsigned char n[32]){
	...
	vec 	vbit, t1;
	gfe1x 	r0w,r0z,r1w,r1z,r2w,r2z;
	gfe1x 	t2,t3,t4,t22;
	gfe 	op_gfe;

	
	r0w.v[0] = _4base[0];   r0w.v[1]=_4base[1];     r0w.v[2]=_4base[2];     r0w.v[3]=_4base[3];
	...
	r2w.v[0] = _4base[0];   r2w.v[1]=_4base[1];     r2w.v[2]=_4base[2];     r2w.v[3]=_4base[3];
	...
	for(i=0;i<31;i++){
			...
			ladderStep(&r0w, &r0z, &r1w, &r1z, &r2w, &r2z);
...
void scalar_mult_var_base(unsigned char op[32], unsigned char n[32], unsigned char varbase[32]){
	...
	gfe1xMult(&t2, &r0w, &t2);
	gfe1xSq(&r0w,&t2);
	r0z.v[0] = _mm_xor_si128(r0w.v[0],d1); 
	r0z.v[1]=r0w.v[1]; r0z.v[2]=r0w.v[2]; r0z.v[3]=r0w.v[3];
		
	gfe1xAdd(&t2, &r0w, &r0z);
	gfe1xMult(&t2, &r0w, &t2);
	gfe1xSq(&r0w,&t2);
	gfe1xnSq(&r0z,&r0z,2);
	gfe1xMultConst(&r0z, &r0z, d1);
	gfe1xAdd(&r0z, &r0w, &r0z);
